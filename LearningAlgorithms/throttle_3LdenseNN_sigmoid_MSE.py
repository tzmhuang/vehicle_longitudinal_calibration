...
#NN FUNCTIONS
def NN_DenseSigmoid(x, W, b, name):
    with tf.name_scope(name):
        a = tf.nn.sigmoid(tf.add(tf.matmul(x,W), b))
    return a

def NeuralNet(x, weights, biases):
    layer_1 = NN_DenseSigmoid(x, weights['layer1'], biases['layer1'], name='Layer_1')
    layer_2 = NN_DenseSigmoid(layer_1, weights['layer2'], biases['layer2'], name='Layer_2')
    layer_3 = NN_DenseSigmoid(layer_2, weights['layer3'], biases['layer3'], name='Layer_3')
...
weights={
    'layer1':tf.Variable(tf.truncated_normal([r_num_, 10], stddev=tf.sqrt(2/r_num_))),
    'layer2':tf.Variable(tf.truncated_normal([10, 10], stddev=tf.sqrt(2/10))),
    'layer3':tf.Variable(tf.truncated_normal([10, 10], stddev=tf.sqrt(2/10))),
    'out':tf.Variable(tf.random_normal([10, 1], stddev=tf.sqrt(2/10)))
    }
...
learning_rate_ = tf.train.exponential_decay(0.0001, global_step, 1000, 0.9, staircase=True)
...
with tf.Session() as sess:
   ...
   ##  rpm_list = [600, 1000, 1400, 1800]
   ##  data_list = []
    feed_data = np.array(feed_data)
    
   ##  for r in rpm_list:
   ##      data_list.append(np.concatenate((feed_data, np.ones((feed_data.shape[0],1))*r), 1))
        
    

   ## a_values = []
   ## for d in data_list:
   ##     a_values.append(sess.run(model_output, feed_dict={X:d}))
   ...
   ##      ax.plot_surface(grid_x, grid_y, a.reshape(grid_x.shape), cmap = cm.coolwarm)
   ##      ax.set_xlabel('v')
   ##      ax.set_ylabel('cmd')
   ##      ax.set_zlabel('a')
   ##      ax.set_title('RPM: '+ str(rpm_list[i]))
